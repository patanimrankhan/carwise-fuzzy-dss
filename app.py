...
DATA_PATH = Path(__file__).parent / "carwise_fuzzy_dataset_55_real.csv"
...
def build_weights(priority, main_use, driving_context, reliability_need):
    ...
    elif main_use == "Adventure / camping":
        weights["condition_score"] += 0.04
        weights["service_history_score"] += 0.03
        weights["accident_safety_score"] += 0.03
    elif main_use == "Work and hauling":
        weights["service_history_score"] += 0.04
        weights["condition_score"] += 0.03
        weights["seller_trust_score"] += 0.02
...
def option_bonus(row, fuel_pref, transmission_pref, car_type_pref, driving_context, main_use):
    ...
    # Body type is now supported by the expanded real dataset. It is still soft, not a hard filter.
    if car_type_pref != "No preference":
        wanted = car_type_pref.lower()
        if wanted == "pickup":
            match = "pick" in body or "pickup" in body
        elif wanted == "suv":
            match = "suv" in body or "off-road" in body
        else:
            match = wanted in body
        if match:
            bonus += 4.0
    ...
    if main_use in ["Adventure / camping", "Work and hauling"]:
        if "pick" in body or "suv" in body or "off-road" in body:
            bonus += 3.2
            notes.append("body type fits adventure, cargo, or outdoor use")
        if "all-wheel" in drive or "4x4" in drive:
            bonus += 2.2
            notes.append("4x4 traction fits outdoor or demanding use")
...
def make_reasons(row, priority, main_use, driving_context):
    """Create more differentiated, car-specific explanation reasons."""
    candidates = []
    ...
    body = str(row.get("body_type", "")).lower()
    drive = str(row.get("drive_type", "")).lower()
    transmission = str(row.get("transmission", "")).lower()
    vfm = float(row.get("value_for_money_indicator", 0) or 0)
    price_score = float(row.get("price_score", 0) or 0)
    mileage_score = float(row.get("mileage_score", 0) or 0)
    age_score = float(row.get("age_score", 0) or 0)
    condition = float(row.get("condition_score", 0) or 0)
    service = float(row.get("service_history_score", 0) or 0)
    trust = float(row.get("seller_trust_score", 0) or 0)
    accident = float(row.get("accident_risk_score", 10) or 10)
    power = float(row.get("power_ps", 0) or 0)
    conf = float(row.get("confidence_score", 0) or 0)

    # Buyer-context reasons get higher weight so two cars do not receive identical generic explanations.
    if priority == "Budget":
        if price_score >= 8: add(11, "Budget fit: the price is favourable compared with the rest of the dataset.")
        if vfm >= 7: add(10, "Good value-for-money: the car balances price, age, mileage, and condition well.")
        if price_score < 5: add(8, "Budget caution: this car is attractive for other reasons, but the price is not its strongest point.")
    elif priority == "Reliability":
        if service >= 8.5: add(12, "Reliability fit: warranty/service evidence is one of this car's strongest signals.")
        if accident <= 1.8: add(11, "Low-risk profile: accident-risk evidence is very low.")
        if condition >= 8.5: add(10, "Condition fit: the vehicle condition score is high for reliability-oriented buyers.")
    else:
        if vfm >= 7: add(10, "Balanced fit: the value-for-money indicator supports this ranking.")
        if condition >= 8 and price_score >= 5: add(9, "Balanced compromise: condition is strong without relying only on low price.")

    if main_use == "City commute":
        if "electric" in fuel or "hybrid" in fuel: add(11, "City use fit: electric/hybrid driving suits short daily routes and traffic.")
        if "automatic" in transmission or "automated" in transmission: add(7, "Comfort fit: automatic transmission is convenient for stop-and-go city driving.")
        if mileage_score >= 7: add(6, "Urban practicality: the mileage profile is still friendly for regular commuting.")
    elif main_use == "Family use":
        if accident <= 2: add(12, "Family fit: low accident-risk evidence supports a safer choice.")
        if condition >= 8: add(10, "Family comfort: strong condition evidence is useful for shared everyday use.")
        if trust >= 8.5: add(8, "Trust signal: the seller/dealer evidence supports a more confident purchase.")
    elif main_use == "Long-distance":
        if service >= 8.5: add(11, "Long-distance fit: strong service/warranty evidence reduces travel-risk concern.")
        if mileage_score >= 7.5: add(9, "Distance fit: the mileage is still reasonable for frequent longer trips.")
        if "electric" in fuel and any(x in str(row.get("consumption_raw", "")).lower() for x in ["500", "570", "590", "583"]): add(9, "EV range fit: the listed range supports longer journeys better than many short-range EVs.")
    elif main_use == "Student budget":
        if price_score >= 8: add(12, "Student-budget fit: the price score is one of this car's strongest advantages.")
        if vfm >= 7: add(10, "Practical value: it offers a useful balance of cost and expected condition.")
    elif main_use in ["Weekend trips", "Adventure / camping"]:
        if "all-wheel" in drive or "4x4" in drive: add(12, "Adventure fit: 4x4/all-wheel drive supports weekend, snow, or hilly trips.")
        if "suv" in body or "off-road" in body or "pick" in body: add(11, "Lifestyle fit: the body type is more suitable for outdoor or flexible use.")
        if service >= 8: add(8, "Trip confidence: service/warranty evidence supports occasional longer journeys.")
    elif main_use == "Work and hauling":
        if "pick" in body: add(13, "Work fit: pickup body style supports cargo and practical hauling needs.")
        if "all-wheel" in drive or "4x4" in drive: add(10, "Utility fit: 4x4 traction is useful for demanding driving contexts.")
        if service >= 8: add(8, "Operational confidence: strong service evidence matters for regular use.")

    if driving_context == "Hilly / snowy areas":
        if "all-wheel" in drive or "4x4" in drive: add(12, "Road-context fit: all-wheel drive is valuable for hilly or snowy areas.")
        if accident <= 2: add(8, "Road safety: low accident-risk evidence is useful in demanding driving conditions.")
    elif driving_context == "Highway":
        if service >= 8.5: add(9, "Highway confidence: strong service evidence supports stable longer driving.")
        if power >= 250: add(5, "Performance reserve: the power profile can be helpful for highway driving.")
    elif driving_context == "City":
        if "electric" in fuel or "hybrid" in fuel: add(9, "City context: electric/hybrid profile can be convenient for urban driving.")

    # General differentiators.
    if age_score >= 8.5: add(8, "Recent vehicle: the age score is strong compared with the dataset.")
    if mileage_score >= 8.5: add(8, "Low-mileage signal: mileage is one of this listing's strongest positives.")
    if condition >= 9: add(8, "High condition score: the vehicle appears especially strong on condition-related evidence.")
    if conf >= 9.5: add(6, "High data completeness: the listing provides enough information for a confident score.")
    if vfm < 5.3: add(5, "Trade-off note: it ranks here mainly due to reliability or fit, not because it is the cheapest value option.")

    # Remove duplicate texts, sort by weight, then choose top three.
    seen = set()
    ordered = []
    for weight, text in sorted(candidates, key=lambda x: x[0], reverse=True):
        if text not in seen:
            ordered.append(text)
            seen.add(text)
    if not ordered:
        ordered = ["The recommendation comes from the combined fuzzy score across price, mileage, age, condition, service history, accident risk, and seller trust."]
    return ordered[:3]
...
def image_data_uri(row):
    """Return a real car image if available; otherwise a clean SVG placeholder."""
    ...
    # Also try by numeric car id: R01 -> images/1.webp and images/01.webp
    car_id = str(row.get("car_id", "")).replace("R", "").lstrip("0") or str(row.get("car_id", ""))
    for name in [car_id, car_id.zfill(2) if car_id.isdigit() else car_id]:
        for ext in [".webp", ".jpg", ".jpeg", ".png"]:
            candidates.append(base_dir / "images" / f"{name}{ext}")
    for p in candidates:
        if p.exists():
            mime = "image/webp" if p.suffix.lower()==".webp" else "image/jpeg" if p.suffix.lower() in [".jpg", ".jpeg"] else "image/png"
            return "data:" + mime + ";base64," + base64.b64encode(p.read_bytes()).decode("utf-8")
    return car_svg()
...
def recommendation_card(row, rank):
    ...
    img_uri = image_data_uri(row)
    body = row.get('body_type','n/a')
    return f"""
    <div class="car-card">
      <div class="rank-badge">#{rank}</div>
      <div style="display:grid; grid-template-columns: 170px 1fr; gap:18px; align-items:center;">
        <img src="{img_uri}" style="width:170px;height:112px;object-fit:cover;border-radius:22px;box-shadow:0 14px 28px rgba(31,41,55,.12);" />
        <div>
          <div class="car-name">{row['car_name']}</div>
          <div class="car-sub">{row.get('fuel_type','')} • {row.get('transmission','')} • {body} • {fmt_chf(row['price_chf'])}</div>
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("**Dataset**")
    st.caption(f"{len(df)} real AutoScout24 cars • sedan, SUV, pickup, hybrid, and electric coverage")
    st.markdown("**Model**")
...
with left:
    ...
    render_option_help(
        ...
        [
            ...
            ("🏕️", "Adventure / camping", "More space, 4x4, towing, and weekend flexibility."),
        ],
    )
    main_use = st.selectbox(
        T["purpose"],
        ["City commute", "Family use", "Long-distance", "Student budget", "Weekend trips", "Adventure / camping", "Work and hauling"],
        index=0,
    ...
    car_type_pref = st.selectbox(
        "Preferred car type",
        ["No preference", "Sedan", "SUV", "Pickup"],
        index=0,
...
""",
    ...
    if car_type_pref in ["Sedan", "SUV", "Pickup"]:
        st.markdown(
            """
<div class="soft-warning">
The selected body type is treated as a strong preference, but not as a hard rule. If few exact matches are available, the app still shows the best alternatives transparently.
</div>
            """,
            unsafe_allow_html=True,
        )
